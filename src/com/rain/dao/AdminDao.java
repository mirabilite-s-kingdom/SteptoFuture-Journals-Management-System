...
public class AdminDao {
	...
	public ArrayList<AdminBean> get_ListInfo() {
		...
		try {
			...
			while (rs.next()) {
				...
				adminbean.setIsVIP(rs.getInt("isVIP"));
	...
	public void updateUser(int aid, String username, String password, String name, String email, String phone,
			int lend_num, int max_num, Double balance, int isVIP) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "update admin set username=?,name=?,email=?,phone=?,password=?,lend_num=?,max_num=?,balance=?,isVIP=? where aid=?";
		PreparedStatement stm = null;
		try {
			stm = conn.prepareStatement(sql);
	...
	public ArrayList<AdminBean> getLikeList(String name) {
		...
		ArrayList<AdminBean> tag_Array = new ArrayList<AdminBean>();
		Connection conn = DBUtil.getConnectDb();
		String sql = "select * from admin where name like '%" + name + "%' or username like '%" + name
				+ "%' or aid like '%" + name + "%'";
		PreparedStatement stm = null;
		ResultSet rs = null;
		try {
	 ...
	 * @return
	 */
	public boolean subBalance(String aid, float borrowPrice) {
