...
<body class="bootstrap-admin-with-small-navbar">
						...
						<table id="data_list" class="table table-hover table-bordered"
							...
							<tbody>
								...
								<td><button type="button" class="btn btn-warning btn-xs"
										...
										'<%=bean.getPhone()%>','<%=bean.getPassword()%>','<%=bean.getLend_num()%>','<%=bean.getMax_num()%>','<%=bean.getBalance()%>',<%=bean.getIsVIP()%>)">修改</button>
    ...
    function showInfo2(aid,username,name,email,phone,password,lend_num,max_num,balance,isVIP) {
        ...
        if(isVIP==0){
            ...
            $("#noVIP").attr("checked","checked");                            
        }else{
        	$("#noVIP").removeAttr("checked");
            $("#isVIP").attr("checked","checked");   
        }    
    }
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								<input type="text" class="form-control" id="updateBalance"
									name="balance" placeholder=""> <label
									class="control-label" for="updatePress" style="display: none;"></label>
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-3 control-label">VIP</label>
							<div class="col-sm-7">
								<input type="radio" name="isVIP" value="1" id="isVIP">是
       							<input type="radio" name="isVIP" value="0" id="noVIP">否
							</div>
						</div>
					</div>
