		...
		ajax(
					  {
			    		...
			    		type:"json",
			    		callback:function(data) {
							// 循环遍历每个期刊分类，每个名称生成一个option对象，添加到<select>中
							for(var index in data) {
								var op = document.createElement("option");//创建一个指名名称元素
								op.value = data[index].typeId;//设置op的实际值为当前的期刊分类编号
...
function validUpdateBook() {
    ...
    if (ISBN == "") {
        ...
        $('#updateISBN').next().text("请输入期刊ISBN码");
    ...
    if (bookName == "") {
        ...
        $('#updateBookName').next().text("请输入期刊名称");
	...
	if(bookType == -1){
        ...
        $('#updateBookType').next().text("请选择期刊分类");
